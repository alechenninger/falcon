...
- Hydration is probably in the low millions-ish of tuples / second range on modest hardware. For a large graph (e.g. 100s of millions) this means 1-2 minutes hydration time. Not ideal for startup latencies. We could potentially reduce this by snapshotting nodes at a consistent LSN in a format that could be more quickly hydrated for just that node's shards. The trick is catching up from that LSN. I had two ideas for this. One is to maintain a secondary replication slot that is manually advanced to known checkpoints. The other is to use another slot that backs up somewhere directly with LSN information, perhaps already organized by node in a format that can be quickly hydrated. Another option is to partition the table to reduce the number of rows scanned per node.