// Package observability provides implementations of domain observer interfaces
// for cross-cutting concerns like logging, metrics, and tracing.
...
func (o *UsersetsObserver) ApplyChangeStarted(ctx context.Context, change domain.Change) (context.Context, domain.ApplyChangeProbe) {
...
func (o *UsersetsObserver) GetSubjectBitmapStarted(key domain.UsersetKey, window domain.SnapshotWindow) domain.BitmapReadProbe {
...
type applyChangeProbe struct {
	domain.NoOpApplyChangeProbe
	logger    *slog.Logger
	ctx       context.Context
	change    domain.Change
	startTime time.Time
	applied   domain.StoreTime
}

func (p *applyChangeProbe) Applied(t domain.StoreTime) {
	p.applied = t
}
...
type bitmapReadProbe struct {
	...
	found     bool
	size      int
	result    domain.SnapshotWindow
}

func (p *bitmapReadProbe) Result(size int, window domain.SnapshotWindow) {
	p.found = true
	p.size = size
	p.result = window
}
...
func (p *containsReadProbe) Result(found bool, window domain.SnapshotWindow) {
	p.found = found
	p.result = window
}
...
type ShardedGraphObserver struct {
	domain.NoOpShardedGraphObserver
	logger *slog.Logger
}
...
func (p *shardedCheckProbe) RemoteCheck(shardID domain.ShardID) {
...
func (p *shardedCheckProbe) UnknownShard(shardID domain.ShardID) {
...
func (p *shardedCheckProbe) Result(found bool, window domain.SnapshotWindow) {
	p.found = found
	p.window = window
}
...
func (p *checkUnionProbe) UnknownShard(shardID domain.ShardID) {
	p.logger.LogAttrs(p.ctx, slog.LevelWarn, "unknown shard",
		slog.String("request_id", RequestIDFromContext(p.ctx)),
		slog.Any("shard_id", shardID))
}

func (p *checkUnionProbe) RemoteShardDispatched(shardID domain.ShardID) {
	if !p.logger.Enabled(p.ctx, slog.LevelDebug) {
		return
	}
...
func (p *checkUnionProbe) RemoteShardResult(shardID domain.ShardID, found bool, window domain.SnapshotWindow) {
	if !p.logger.Enabled(p.ctx, slog.LevelDebug) {
		return
	}
...
func (p *checkUnionProbe) RemoteShardError(shardID domain.ShardID, err error) {
...
func (p *checkUnionProbe) Result(found bool, window domain.SnapshotWindow) {
	p.found = found
	p.window = window
}

func (p *checkUnionProbe) Inconclusive(failedShards []domain.ShardID) {
...
func (p *checkProbe) CycleDetected(key domain.VisitedKey) {
	if !p.logger.Enabled(p.ctx, slog.LevelDebug) {
		return
	}
...
func (p *checkProbe) DirectLookupResult(found bool, window domain.SnapshotWindow) {
	if !p.logger.Enabled(p.ctx, slog.LevelDebug) {
		return
	}
...
func (p *checkProbe) Result(found bool, window domain.SnapshotWindow) {
	p.found = found
	p.window = window
}
...
func (p *localCheckProbe) Result(found bool, window domain.SnapshotWindow) {
	p.found = found
	p.window = window
}
...
func (p *localCheckUnionProbe) BitmapLookupResult(size int, window domain.SnapshotWindow) {
	if !p.logger.Enabled(p.ctx, slog.LevelDebug) {
		return
	}
...
func (p *localCheckUnionProbe) ContainsCheckResult(found bool, window domain.SnapshotWindow) {
	if !p.logger.Enabled(p.ctx, slog.LevelDebug) {
		return
	}
...
func (p *localCheckUnionProbe) Result(result domain.CheckResult) {
	p.result = result
}
...
func (o *MVCCObserver) SnapshotWithinStarted(maxTime domain.StoreTime) domain.MVCCProbe {
	return &mvccProbe{
		logger:    o.logger,
...
func (p *mvccProbe) Result(found bool, stateTime domain.StoreTime) {
	p.found = found
