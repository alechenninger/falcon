...
func (g *ShardedGraph) ReplicatedTime() StoreTime {
...
func (g *ShardedGraph) SetReplicatedTime(t StoreTime) {
...
func (f *filteringTupleIterator) Tuple() Tuple {
	return f.current
}
		...
		for {
			...
			case change, ok := <-innerChanges:
				...
				if !f.include(change.Tuple.ObjectType, change.Tuple.ObjectID) {
					change.Tuple = Tuple{} // Clear tuple data
				}
				select {
				case changes <- change:
				case <-ctx.Done():
...
func (f *filteringChangeStream) CurrentTime(ctx context.Context) (StoreTime, error) {
	return f.inner.CurrentTime(ctx)
}

// Compile-time interface checks for filtering types
var (
	_ TupleIterator = (*filteringTupleIterator)(nil)
	_ ChangeStream  = (*filteringChangeStream)(nil)
)
