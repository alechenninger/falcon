...
var testSchema = func() *domain.Schema {
	s := &domain.Schema{
		Types: map[domain.TypeName]*domain.ObjectType{
			"user":     {ID: 1, Name: "user", Relations: map[domain.RelationName]*domain.Relation{}},
			"group":    {ID: 2, Name: "group", Relations: map[domain.RelationName]*domain.Relation{}},
			"document": {ID: 3, Name: "document", Relations: map[domain.RelationName]*domain.Relation{}},
			"folder":   {ID: 4, Name: "folder", Relations: map[domain.RelationName]*domain.Relation{}},
		},
	}
	s.Types["group"].Relations["member"] = &domain.Relation{ID: 1, Name: "member"}
	s.Types["document"].Relations["viewer"] = &domain.Relation{ID: 1, Name: "viewer"}
	s.Types["document"].Relations["editor"] = &domain.Relation{ID: 2, Name: "editor"}
	s.Types["folder"].Relations["viewer"] = &domain.Relation{ID: 1, Name: "viewer"}
	s.Compile()
	return s
}()

// collectTuples is a test helper that collects all tuples from an iterator.
func collectTuples(t *testing.T, iter domain.TupleIterator) []domain.Tuple {
	t.Helper()
	defer iter.Close()
...
// setupPostgres creates a Postgres container and returns a connected Postgresdomain.
// The container is automatically cleaned up when the test ends.
func setupPostgres(t *testing.T) *infrapostgres.Store {
	t.Helper()
...
func TestPostgresStore_LoadAll_Multiple(t *testing.T) {
	...
	loadedMap := make(map[domain.Tuple]bool)
...
func TestPostgresStore_UsersetSubject(t *testing.T) {
	...
	directTuple := domain.Tuple{
		ObjectType:      testSchema.GetTypeID("document"),
		ObjectID:        100,
		Relation:        testSchema.GetRelationID("document", "viewer"),
		SubjectType:     testSchema.GetTypeID("user"),
		SubjectID:       1,
		SubjectRelation: domain.NoRelation, // 0 = direct subject
	}

	// Userset subject: document:100#viewer@group:1#member
	usersetTuple := domain.Tuple{
		ObjectType:      testSchema.GetTypeID("document"),
		ObjectID:        100,
		Relation:        testSchema.GetRelationID("document", "viewer"),
		SubjectType:     testSchema.GetTypeID("group"),
